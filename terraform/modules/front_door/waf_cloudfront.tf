# When updating this cloudfront WAF acl, consider making the same change for the load balancer WAF acl, as they generally need to be the same
      ...
      rate_based_statement {
        ...
        scope_down_statement {
          regex_pattern_set_reference_statement {
            arn = aws_wafv2_regex_pattern_set.waf_cloudfront_rate_limit_urls.arn
...
# When updating this cloudfront WAF regex pattern, consider making the same change for the load balancer WAF regex pattern, as they generally need to be the same
resource "aws_wafv2_regex_pattern_set" "waf_cloudfront_rate_limit_urls" {
  name     = "${var.prefix}-waf-cloudfront-login-url-regex-patterns"
  provider = aws.us-east-1
  scope    = "CLOUDFRONT"
